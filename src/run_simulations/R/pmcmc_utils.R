...
#' @noRd
generate_draws_inner <- function(out, pars.list, draws = 10, parallel, interventions = NULL){
...
generate_draws <- function(out, pars.list, draws = 10, parallel = TRUE, noInfectionProtect = FALSE) {
  ...
  r <- generate_draws_inner(out, pars.list, draws, parallel)
  return(r)
}
...
deaths_averted <- function(out, draws, parallel, counterfactual = "No Vaccine", reduce_age = TRUE,
  ...
  baseline <- generate_draws(out, pars.list, draws, parallel)
  ...
  if(direct){
    baseline_direct <- generate_draws(out, pars.list, draws, parallel, noInfectionProtect = TRUE)
    # format the counter factual run
