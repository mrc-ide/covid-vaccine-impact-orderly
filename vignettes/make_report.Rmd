...
Originally we get fits from an orderly repository stored elsewhere then generate
our simulations of deaths averted. However, we now run these on the DIDE cluster
so an orderly task is used to gather these from a shared directory. These tasks
are still included however this vignette will not run then as the results should
be included in the GitHub repo. This is also true for the online resources used,
the leaflet world map and Our World in Data. All other tasks should only require
access to other tasks in this repository.
